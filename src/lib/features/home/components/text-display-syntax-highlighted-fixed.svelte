<script lang="ts">
	...
	$effect(() => {
		if (currentText) {
			...
			try {
				highlightedChars = highlightCode(
					currentText,
					detectedLang as keyof typeof LANGUAGE_PATTERNS
				);
			} catch (error) {
		...
		if (currentCharIndex !== lastCurrentIndex && currentCharIndex !== -1) {
			...
			setTimeout(() => {
				if (containerRef && currentCharRef) {
					...
					if (
						charRect.bottom > containerRect.bottom + 20 ||
						charRect.top < containerRect.top - 20
					) {
...
<div
	...
	class="mt-4 max-h-[400px] select-none overflow-auto rounded-lg border border-gray-300 bg-gray-50 p-4 font-mono text-lg leading-relaxed shadow-sm dark:border-gray-600 dark:bg-gray-900"
	...
	{#each charsWithStatus as charData, charIndex}
		...
		{#if isCurrent}
			<span
				...
				class="relative {colorClass} bg-blue-200 text-blue-900 dark:bg-blue-800 dark:text-blue-100"
				style="box-shadow: 0 0 0 1px rgba(59, 130, 246, 0.5);"
				>{char === '\n' ? '\n' : char === '\t' ? '\t' : char}</span
			>
		{:else}
			<span
				class="{status === 'correct' ? 'bg-green-100 dark:bg-green-900/30' : ''} {status ===
				'incorrect'
					? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'
					: ''} {colorClass} {status === 'pending' ? 'opacity-70' : ''}"
				>{char === '\n' ? '\n' : char === '\t' ? '\t' : char}</span
			>
		{/if}
	{/each}
